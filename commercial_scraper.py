...
def extract_reports(reports):
    ...
    for index, report in enumerate(reports):
        '''
        get report url from html attribute, doing this because the page lazy loads i.e.
        does not load the html for all items at once
        '''
        reports[index] = report.get('data-deferred-loaded') or report.get('data-deferred-load')
        print index, "/", num_reports, ": ", reports[index]
        extract_report(reports[index])
...
def write_to_csv(filename, results):
    ...
    column_headings = sorted(results[0].keys())
